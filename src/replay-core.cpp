...
void ReplayCore::loadConfigFile(const char *path, bool projectScoped)
{
	...
	// §7.3.11 — missing means OFF: a config.json written before this key
	// existed never had Space do anything, and that has to stay true.
	config_.spacebarPlays = obs_data_get_bool(data, "spacebarPlays");
...
void ReplayCore::saveConfigFile(const char *path) const
{
	...
	obs_data_set_bool(data, "spacebarPlays", config_.spacebarPlays);
